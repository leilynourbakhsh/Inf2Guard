...
def make_loaders(workers, batch_size, transforms, data_path, args, data_aug=True,
    ...
    **INTERNAL FUNCTION**
    ...
    if not custom_class:
        ...
        if not only_val:
            try:
                if args.dataset == 'letters':
                    train_set = custom_class(root=data_path, train=False, split='letters',
        ...
        try:
            if args.dataset == 'letters':
                test_set = custom_class(root=data_path, train=False, split='letters',
                                        download=True, transform=transform_test)
            else:
