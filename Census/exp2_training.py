...
def train_FE_CF(FE, CF, data_train_loader, current_lr, vis=None):
    ...
    for ind, (X, (privlabels, labels)) in enumerate(data_train_loader):
        ...
        for i in range(4):
            ...
            single_sample=add_gaussian_noise(single_sample)
...
def get_FE_CF():
    ...
    if torch.cuda.device_count() > 1:
        torch.save(FE.module, os.path.join(save_dir, "FE_noise.pth"))
        torch.save(CF.module, os.path.join(save_dir, "CF_noise.pth"))
    else:
        torch.save(FE, os.path.join(save_dir, "FE_noise.pth"))
        torch.save(CF, os.path.join(save_dir, "CF_noise.pth"))
...
def plot_tsne_3d(features, labels, figsize=(5, 5), point_size=6):
    # Get indices for all classes
    unique_labels = np.unique(labels)
    selected_indices = []
    num_samples_per_class = min([len(np.where(labels == lbl)[0]) for lbl in unique_labels])
    
    for lbl in unique_labels:
        lbl_indices = np.where(labels == lbl)[0]
        selected_indices.extend(np.random.choice(lbl_indices, num_samples_per_class, replace=False))
    ...
    for lbl in unique_labels:
        ax.scatter(reduced_features[selected_labels == lbl, 0], 
                   reduced_features[selected_labels == lbl, 1], 
                   reduced_features[selected_labels == lbl, 2], 
